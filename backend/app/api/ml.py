...
def _debug_log(message: str, data: dict, hypothesis_id: str, run_id: str = "run1") -> None:
    ...
    # File sink is opt-in via DEBUG_LOG_PATH (dev only). In production it is unset, so we
    # never write stray debug files to disk — the structured log line above is enough.
